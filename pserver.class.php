...
class pserver
{
	...
	*/
	 ...
	 * Message welcome
	*/
	var $welcome_data = "Welcome my Friendnr";
	/**
	 * @var $msg_full 
	...
	*/
	...
	var $full_welcome = "To many Clients connected!nr";
	/**
	 * @var $bufferin 
	 * Buffer in
	*/
	...
	*/
	...
	{		
		//$this->data[ $client['n'] ] = base64_encode(trim(@socket_read($client['socket'], $this->bufferin, (  $this->socketbinary ?  PHP_BINARY_READ : PHP_NORMAL_READ  ) )))."nr";
		//$this->data[ $client['n'] ] = trim(@socket_read($client['socket'], $this->bufferin, (  $this->socketbinary ?  PHP_BINARY_READ : PHP_NORMAL_READ  ) ))."\n\r";
		$this->data[ $client['n'] ] = trim(@socket_read($client['socket'], $this->bufferin, (  $this->socketbinary ?  PHP_BINARY_READ : PHP_NORMAL_READ  ) ));
		$valor=$this->data[ $client['n'] ];
		//echo $valor;
		$a=$this->graba($valor);
		//var_dump($a);
	}	
	function graba($palabra)
	{
		if ( is_string($palabra) && strlen($palabra)>0 )
		{
			//echo "XXXXXXXXXXXXXXXXXXXXXXXXXXXX".$palabra."\n";
			require_once('funciones_logger.php');
			$etdo=Bit2File($palabra);
			echo "Codigo ".$etdo." para la palabra ".$palabra."\n";
			/*$global=new Conecciones();
			$DB=$global->db_local_v1;
			$status=$DB->Conectar("bolas_sag");
			$sql="INSERT INTO temp_test (valor) values ('".$palabra."')";
			//echo $sql;
	...
	*/
	...
	{
		print date("[Y-m-d H:i:s]")."t".$area."t".$msg."\n\r";
	}	
	/**
