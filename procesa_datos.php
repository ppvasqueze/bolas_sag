...
function procesa()
{
	...
	1	-> Calculando
	2	-> Procesado bien
	3	-> No procesado por cálculo sobre datos de otro perno
	-1	-> Error
	*/
	$ERROR=false;
	//Aceleracion g
...
function graba_estado($id_reg, $estado_futuro)
{
	...
	if ($estado_futuro==2)
	{
		//Cambiamos además todos los de la misma marca de tiempo (mismo timetick y bola, distinto perno)
		$sql="UPDATE parametros_mem SET status=3 
			WHERE	idbola	= (SELECT idbola FROM parametros_mem WHERE idregistro=".$id_reg.")
			AND		timetick	= (SELECT timetick FROM parametros_mem WHERE idregistro=".$id_reg.")";
		$resultado=$DB->consultaSQL($sql, false);
	}
